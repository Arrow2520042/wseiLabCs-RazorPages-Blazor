...
public class PersonService(IContactUnitOfWork unitOfWork, IMapper mapper) : IPersonService
{
    ...
    {
        return await GetById(personId)
               ?? throw new ContactNotFoundException($"Person with id {personId} was not found.");
    }
    ...
    public async Task RemoveNoteFromPerson(Guid personId, Guid noteId)
    {
        ...
        var note = person.Notes.FirstOrDefault(n => n.Id == noteId);
        if (note is null)
            throw new NoteNotFoundException($"Note with id {noteId} was not found for person {personId}.");
